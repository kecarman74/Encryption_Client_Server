...
char* enc_msg(char* key, char* msg) {
    char buffer[90000];
    int i;
    int key_int;
    int msg_int;
        ...
        switch (spawnPid) {
            ...
            case 0:
                ...
                //check if key is long enough
                    ...
                    encrypted_msg = enc_msg(key, msg);

                    int chars_sent = 0;
                    int enc_msg_len = strlen(encrypted_msg);

                    while (chars_sent < enc_msg_len) {
                        // Send encrypted message back to the client
                        charsRead = send(connectionSocket,
                            encrypted_msg, strlen(encrypted_msg), 0);
                        if (charsRead < 0) {
